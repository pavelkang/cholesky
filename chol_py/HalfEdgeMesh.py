...
class HalfEdgeMesh(object):
    ...
    def printMeshStats(self, printImaginary=False):
        ...
        # TODO check if mesh has positions before doing this
        print('\n   Edge length statistics:')
        edgeLens = []
        for edge in self.edges:
            edgeLens.append(norm(edge.anyHalfEdge.vec))
        maxLen = max(edgeLens)
        minLen = min(edgeLens)
        print('    - min edge length = ' + str(minLen))
        print('    - max edge length = ' + str(maxLen))
...
class HalfEdge(object):
    ...
    def oppAngle(self):
        ...
        A = -normalize(self.next.vec)
        B = normalize(self.next.next.vec)
    ...
    def edgeAngle(self):
        ...
        sourceVert = self.twin.vertex
        angSum = 0.0
        currOutEdge = sourceVert.referenceEdge
        while currOutEdge is not self:
            # TODO can optimize this loop

            nextEdge = currOutEdge.twin.next
            A = normalize(currOutEdge.vec)
            B = normalize(nextEdge.vec)

            # Clamp the dot product to deal with numerical error, -1.00...002 => -1.0
            dotProd = clamp(np.dot(A,B),lower=-1.0,upper=1.0)
    ...
    def transportAngle(self):
        ...
        """
        angle = regAngle(self.twin.edgeAngleRescaled - self.edgeAngleRescaled + pi)
...
class Vertex(object):
    ...
    # TODO Implement some more careful treatment of numpy arrays to help avoid bugs
    #   - Values like position (and others) whould have the writable flag set to False
    #   - Make a (unwritable?) copy of an array when we stick in in the cache
    ...
    def angleDefect(self):
        ...
        if(self.isBoundary):
            ...
            angSum = 0.0
            vList = [normalize(h.vec) for h in self.adjacentHalfEdges()]
    ...
    @cacheGeometry
    def dualArea(self):
        """The dual area for this vertex, 1/3 the sum of adjacent face areas"""
        return 1/3.0 * sum([face.area for face in self.adjacentFaces()])


    @property
    @cacheGeometry
    def referenceDirectionR3(self):
        """
        Computes a reference direction ('referenceDirectionR3') in R3, guaranteed to
        ...
        r = self.referenceEdge.vec
        rNorm = np.dot(self.normal, r)*self.normal
        rInPlane = normalize(r - rNorm)
        referenceDirectionR3 = rInPlane

        return referenceDirectionR3
...
class Face(object):
    ...
    ### TODO All of these are only valid on triangular faces, but that is not checked ###
    ...
    def center(self):
        ...
        v1 = self.anyHalfEdge.vertex.pos
        v2 = self.anyHalfEdge.next.vertex.pos
        v3 = self.anyHalfEdge.next.next.vertex.pos
...
class Edge(object):
    ...
    def cotanLaplace(self):
        ...
        If called on a boundary edge, only gives the cotan component for the interior face
        """

        if self.isBoundary:
            return 0.5 * self.anyHalfEdge.cotan

        val =  0.5 * (self.anyHalfEdge.cotan + self.anyHalfEdge.twin.cotan)
        return val
